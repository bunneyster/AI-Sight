...
class OverlayingTexturesGenerater: NSObject {
    ...
    private func generateTextureBuffer(
        ...
        _ targetWidth: Int,
        _ targetHeight: Int,
        _ rotation: MetalVideoView.Rotation
    )
        -> MTLBuffer?
    {
        ...
        let finalCoordinates = rotation == .rotate90Degrees ? rotatedCoordinates : coordinates

        let textureBuffer = sharedMetalRenderingDevice.device.makeBuffer(
            bytes: finalCoordinates,
            length: finalCoordinates
                .count *
    ...
    func texture(_ source1: Texture, _ source2: Texture) -> Texture? {
        ...
        if textureBuffer1 == nil {
            textureBuffer1 = generateTextureBuffer(
                source1.texture.width,
        ...
        if textureBuffer2 == nil {
            textureBuffer2 = generateTextureBuffer(
                source2.texture.width,
                source2.texture.height,
                minX,
                minY,
                .rotate0Degrees
            )
